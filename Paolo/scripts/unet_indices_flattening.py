...
#       'NAO_classic', 'PNA', 'NPD'])
    ...
    for lag in range(14):
        if lag==0:
            ...
            model.fit([train_img_std_padded,indices_95.iloc[0:11323,1:]], y_train_img, validation_data=([val_img_std_padded,indices_95.iloc[11323:13149,1:]],y_val_img),
            callbacks=[monitor],epochs=200)
            #t = model.predict([test_img_std_padded,])
            t = model.predict([test_img_std_padded,indices_95.iloc[13240:-244,1:]])
            ...
            #            callbacks=[monitor],epochs=100)
            model.fit([train_img_std_padded[:-lag],indices_95.iloc[0:11323-lag,1:]], y_train_img[lag:], validation_data=([val_img_std_padded[:-lag],indices_95.iloc[11323:13149-lag,1:]],y_val_img[lag:]),
            callbacks=[monitor],epochs=200)
            #t = model.predict(test_img_std_padded)
            t = model.predict([test_img_std_padded,indices_95.iloc[13240:-244,1:]])
            ### Brier score ### 
