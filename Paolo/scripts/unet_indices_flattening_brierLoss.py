...
def simple_unet_model_flatten(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS):
#Build the model
...
def simple_unet_model_flattenIndices(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS):
#Build the model
    input_img = Input((IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
    #s = Lambda(lambda x: x / 255)(inputs)   #No need for this if we normalize our inputs beforehand
    s = input_img
    ...
    outputs = Conv2D(1, (1, 1), activation='linear')(c10) #sigmoid
...
#       'NAO_classic', 'PNA', 'NPD'])
    ...
    y_train_img = np.load('/home/b/b382633/TC_adds/predictions/Unet/Sindiany_train_img_noBinary.npy')/100
    y_val_img = np.load('/home/b/b382633/TC_adds/predictions/Unet/Sindiany_val_img_noBinary.npy')/100
    y_test_img = np.load('/home/b/b382633/TC_adds/predictions/Unet/Sindiany_test_img_noBinary.npy')/100
    ...
    for lag in range(14):
        if lag==0:
            #model = simple_unet_model_flatten(16,32,9)
            model = simple_unet_model_flattenIndices(16,32,9)
            ...
            model.fit([train_img_std_padded,indices_99.iloc[0:11323,1:]], y_train_img, validation_data=([val_img_std_padded,indices_99.iloc[11323:13149,1:]],y_val_img),
            callbacks=[monitor],epochs=200)
            #t = model.predict([test_img_std_padded,])
            t = model.predict([test_img_std_padded,indices_99.iloc[13240:-244,1:]])
            ...
            #            callbacks=[monitor],epochs=100)
            model.fit([train_img_std_padded[:-lag],indices_99.iloc[0:11323-lag,1:]], y_train_img[lag:], validation_data=([val_img_std_padded[:-lag],indices_99.iloc[11323:13149-lag,1:]],y_val_img[lag:]),
            callbacks=[monitor],epochs=200)
            #t = model.predict(test_img_std_padded)
            t = model.predict([test_img_std_padded,indices_99.iloc[13240:-244,1:]])
            ### Brier score ### 
