...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", default='/Users/paolo/Documents/TC/Paolo/data/mutiple_regions/')
    parser.add_argument("--features_list", default=['vo','r','u_200','u_850','v_200','v_850','tcwv','sst','shear'])#, 'NAOP_S', 'BLOCK_S', 'NAOM_S', 'RIDGE_S', 'nino3', 'nino4',
    ...
    parser.add_argument("--savepath", default='/Users/paolo/Documents/TC_predictions/multiple_areas/')
    ...
    for lag in range(14):
        if lag == 0:
            if args.model=='CNN':
                ...
                model.add(layers.Conv2D(out_dim, (3, 3), activation='sigmoid', padding='same')) # sigmoidal activation to have probabilities
                ...
                model.fit(train_img_std[2:-2], np.concatenate((y_train_img[2:-2],y_train_img[3:-1],y_train_img[4:]),axis=3), validation_data=(val_img_std[2:-2],np.concatenate((y_val_img[2:-2],y_val_img[3:-1],y_val_img[4:]),axis=3)),
            ...
            if args.model=='Unet':
                ...
                model.fit(train_img_std_padded[2:-2], np.concatenate((y_train_img[2:-2],y_train_img[3:-1],y_train_img[4:]),axis=3), validation_data=(val_img_std_padded[2:-2],np.concatenate((y_val_img[2:-2],y_val_img[3:-1],y_val_img[4:]),axis=3)),
                            callbacks=[monitor],epochs=100)
            ...
            #    print(f'Model Brier score 1: {brier_score_loss(y_test_img_curr.reshape(-1,1), t_curr.reshape(-1,1))}')  
                ...
                if args.save=='yes':
                    x_test[i]['predictions_lag0_0'] = t_curr[:,:,:,0].reshape(-1,1)
                    x_test[i]['predictions_lag1_0'] = t_curr[:,:,:,1].reshape(-1,1)
                    x_test[i]['predictions_lag2_0'] = t_curr[:,:,:,2].reshape(-1,1)
            ...
            if args.model=='CNN':
                ...
                model.fit(train_img_std[2:-2-lag], np.concatenate((y_train_img[lag+2:-2],y_train_img[lag+3:-1],y_train_img[lag+4:]),axis=3), validation_data=(val_img_std[2:-2-lag],np.concatenate((y_val_img[lag+2:-2],y_val_img[lag+3:-1],y_val_img[lag+4:]),axis=3)),
                            callbacks=[monitor],epochs=100)
            ...
            if args.model=='Unet':
                ...
                model.fit(train_img_std_padded[2:-2-lag], np.concatenate((y_train_img[lag+2:-2],y_train_img[lag+3:-1],y_train_img[lag+4:]),axis=3), validation_data=(val_img_std_padded[2:-2-lag],np.concatenate((y_val_img[lag+2:-2],y_val_img[lag+3:-1],y_val_img[lag+4:]),axis=3)),
                            callbacks=[monitor],epochs=100)
            ...
            for i in range(len(areas)):
                ...
                z = np.zeros((lag,13,29))
                if args.save=='yes':
                    x_test[i]['predictions_lag0_'+str(lag)] = np.concatenate((t_curr[:,:,:,0],z)).reshape(-1,1)
                    x_test[i]['predictions_lag1_'+str(lag)] = np.concatenate((t_curr[:,:,:,1],z)).reshape(-1,1)
                    x_test[i]['predictions_lag2_'+str(lag)] = np.concatenate((t_curr[:,:,:,2],z)).reshape(-1,1)
                    x_test[i].to_csv(args.savepath+areas[i]+'.csv')
